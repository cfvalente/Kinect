 ...
 *
 * $Id: TrackerImpl.ipp 226 2007-09-14 06:44:18Z bornik $
 * @file
 * ======================================================================== */
...
#pragma message(">>> using Division LUT")
	// TODO: should be allocated from memory manager!
	DIV_TABLE = new unsigned short[DIV_TABLE_SIZE];

	for(int i=DIV_TABLE_MIN*DIV_TABLE_PREC; i<DIV_TABLE_MAX*DIV_TABLE_PREC; i++)
	{
		int v = (i>0) ? (DIV_TABLE_BASE_FACT<<16)/i : 0xffff;
		int idx = i - DIV_TABLE_PREC*DIV_TABLE_MIN;
		assert(idx>=0 && idx<DIV_TABLE_SIZE);
...
AR_TEMPL_TRACKER::changeCameraSize(int nWidth, int nHeight)
{
	...
	if(logger)
		logger->artLogEx("ARToolKitPlus: Changed CamSize %d, %d\n", arCamera->xsize, arCamera->ysize);
}
