								  ...
								  ARFloat **pos, int **clip, int **label_ref)
{
	...
	int corrLeftY = (vignetting.corners*threshFact)<<shiftBits,
		...
		corrX = 0, dCorrX = 0,
#endif
	int	corrThresh = thresh;

	for(j = 1; j < lysize-1; j++, pnt+=poff*2, pnt2+=2)
	{

#ifdef LABERLING_PREFETCH_LINE
		// prefetch data for a line into cache
		//
		unsigned long *srcI0 = (unsigned long *)pnt, *srcI;
		srcI = srcI0 + arImXsize/4 - 64;
		while(srcI>=srcI0)
		{
			_PreLoad(srcI+0);
			_PreLoad(srcI+16);
