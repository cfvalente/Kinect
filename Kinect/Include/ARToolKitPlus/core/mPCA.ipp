 ...
 *
 * $Id: mPCA.ipp 210 2007-05-09 12:26:23Z bornik $
 * @file
 * ======================================================================== */
...
AR_TEMPL_TRACKER::arMatrixPCA(ARMat *input, ARMat *evec, ARVec *ev, ARVec *mean)
{
	...
	// 2.02 ms on the Tornado
	{
		float inv_srow = 1.0f/srow;
		for(i=0; i<row*clm; i++)
			work->m[i] *= inv_srow;

		//PROFILE_BEGINSEC(profiler, MATRIXPCA)
		rval = PCA( work, evec, ev );
		//PROFILE_ENDSEC(profiler, MATRIXPCA)
		Matrix::free( work );
...
PCA( ARMat *input, ARMat *output, ARVec *ev )
{
    ...
    if( row < clm ) {
		// 88 float multiplications (4 times per marker)
    ...
    if( row < clm ) {
        ...
        m2 = output->m;
	for( i = 0; i < min; i++){
	    if( ev->v[i] < (Matrix::FLOAT)VZERO ) break;
